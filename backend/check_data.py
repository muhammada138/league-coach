    ...
    try:
        with sqlite3.connect(DB_PATH) as conn:
            ...
            if "training_matches_v1" in tables:
                count = conn.execute("SELECT COUNT(*) FROM training_matches_v1").fetchone()[0]
                print(f"✅ Table 'training_matches_v1': {count} rows (Legacy Data)")
                if count > 0:
                    print("   ✨ This is likely where your 11k data points are stored!")
            else:
                print("ℹ️ Table 'training_matches_v1' not found. (Migration might not have run yet)")

            # Check ingestion status
            if "ingestion_status" in tables:
                status = conn.execute("SELECT processed_count, total_target, is_paused FROM ingestion_status WHERE id = 1").fetchone()
                if status:
                    print("-" * 40)
