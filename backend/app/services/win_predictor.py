...
def _player_features(stats: dict, champion_id: int):
    ...
    if not stats or (
        tier == "UNRANKED" and wins == 0 and losses == 0 and not stats.get("last5")
    ):
        return None
    ...
    if tier == "UNRANKED":
        ...
        rank_score = (
            min((tier_val + div_val + lp_bonus) / MAX_RANK, 1.2) / 1.2
        )  # Allow some headroom
...
def predict(participants: list[dict], live_stats: dict) -> dict:
    ...
    blue_feats = [
        _player_features(live_stats.get(p.get("puuid", ""), {}), p.get("championId", 0))
        for p in blue_raw
    ]
    red_feats = [
        _player_features(live_stats.get(p.get("puuid", ""), {}), p.get("championId", 0))
...
def retrain_on_real_data() -> dict:
    ...
    if not _ML_DEPS_AVAILABLE:
        return {
            "ok": False,
            "error": "ML dependencies (joblib, xgboost) are not installed.",
        }
