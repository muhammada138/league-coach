...
def load_or_train_model() -> None:
    ...
    try:
        if MODEL_PATH.exists():
            ...
            logger.info("No saved model – using linear fallback until retrained.")
    except Exception as exc:
        logger.warning("Model load failed (%s) – using linear fallback", exc)


# Removed synthetic generation completely as per user request.
...
def retrain_on_real_data() -> dict:
    ...
    try:
        ...
        if len(clean_rows) >= 500:
            ...
            return {"ok": False, "error": f"Not enough real data yet (found {len(clean_rows)}) to train."}
