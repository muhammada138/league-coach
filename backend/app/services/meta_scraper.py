    ...
    async with httpx.AsyncClient(timeout=20.0) as client:
        try:
            ...
            for opp_name, opp_cid in _CHAMP_ID_MAP.items():
                ...
                # Look for opponent name then the VS winrate decimal
...
async def fetch_rank_meta(rank: str) -> dict:
    """Scrape all 5 lane tierlists to ensure we find every single champion."""
    ...
    logger.info("Exhaustively found %d champions for rank %s", len(results["champions"]), rank)
    return results
...
async def sync_meta():
    ...
    try:
        ...
        # 1. Update Global Tierlists
        for rank in RANKS:
            if _cancel_requested: break
            while _sync_paused and not _cancel_requested: await asyncio.sleep(1.0)
        ...
        # 2. Deep Matchup Sync (Incremental)
        import random
