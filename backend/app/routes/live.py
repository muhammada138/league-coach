...
def _assign_duo_groups(results: list[dict]):
    ...
    return {r["puuid"]: r for r in results if r and "puuid" in r}
...
async def live_enrich(body: LiveEnrichRequest):
    ...
    results = await asyncio.gather(*[_enrich_player(p, body, rank_type, is_ranked_queue, match_queue_filter) for p in body.puuids[:10]])
    return _assign_duo_groups(results)
