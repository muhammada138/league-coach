...
@router.get("/teammates/{puuid}")
async def get_teammates(puuid: str):
    """
    Fetches last 100 games (Solo/Duo -> Flex -> Normal) and aggregates 
    teammates/opponents found more than twice.
    """
    async with httpx.AsyncClient(timeout=60.0) as client:
        # Try Solo/Duo first, then Flex, then Normal
        queue_priorities = [420, 440, 400]
        match_ids = []
        for q in queue_priorities:
            ids = await riot_get(client, f"https://{RIOT_ROUTING}.api.riotgames.com/lol/match/v5/matches/by-puuid/{puuid}/ids?count=100&queue={q}")
            if ids:
                match_ids = ids
                break
        ...
        # Fetch match details in chunks to avoid overwhelming or rate limits
        chunk_size = 20
        all_matches = []
        for i in range(0, len(match_ids), chunk_size):
            chunk = match_ids[i:i+chunk_size]
            tasks = [riot_get(client, f"https://{RIOT_ROUTING}.api.riotgames.com/lol/match/v5/matches/{mid}") for mid in chunk]
            results = await asyncio.gather(*tasks, return_exceptions=True)
            all_matches.extend([r for r in results if not isinstance(r, Exception)])
    ...
    for match in all_matches:
        ...
        for p in participants:
            p_puuid = p.get("puuid")
            if p_puuid == puuid: continue
            
            name = p.get("riotIdGameName") or p.get("summonerName") or "Unknown"
            tag = p.get("riotIdTagline") or ""
            
            target_map = with_map if p.get("teamId") == my_team_id else against_map
            if p_puuid not in target_map:
                target_map[p_puuid] = {"puuid": p_puuid, "name": name, "tag": tag, "games": 0, "wins": 0}
            
            target_map[p_puuid]["games"] += 1
            if win:
                target_map[p_puuid]["wins"] += 1

    def finalize(m):
        # Only return those with more than 1 game (>= 2) as requested ("more than 2" might mean >= 2 or > 2)
        # User said "who haev more than 2 games played tgt like it is"
        # In current UI it shows all, but user wants to find "patterns".
        # I'll return all but maybe the UI should filter? 
        # User specifically said "with teammates who haev more than 2 games played tgt"
        # I'll return everything sorted by games played, and maybe the UI handles the threshold.
        res = sorted(m.values(), key=lambda x: x["games"], reverse=True)
        return [x for x in res if x["games"] >= 2]

    return {
        "with": finalize(with_map),
        "against": finalize(against_map)
    }
