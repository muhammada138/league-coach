...
# Let's cleanly replace the _predict_probability function definition
correct_func = """def _predict_probability(blue_vec: np.ndarray, red_vec: np.ndarray, diff_vec: np.ndarray) -> float:
...
content = re.sub(r'def _predict_probability.*?\n\nasync def', correct_func + "\n\nasync def", content, flags=re.DOTALL)
